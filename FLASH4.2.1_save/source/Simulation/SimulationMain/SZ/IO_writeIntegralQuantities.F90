...
subroutine IO_writeIntegralQuantities ( isFirst, simTime)
  ...
  use Simulation_data, ONLY: sim_rvir
  implicit none
...
#include "Flash.h"
  ...
  integer, parameter ::  nGlobalSumProp = 9              ! Number of globally-summed regular quantities
  ...
  integer :: sizeX
  integer :: istat
  logical :: gcell = .true. !ES
  real,allocatable, dimension(:) ::xCenter !ES
  ...
  do lb = 1, count
     ...
     sizeX = blkLimitsGC(HIGH,IAXIS)     !ES
     if (lb.eq.1) allocate(xCenter(sizeX),stat=istat) !ES
     call Grid_getCellCoords(IAXIS, blockList(lb), CENTER, gcell, xCenter, sizeX)
...
#endif
!ES carry out a sum within the virial radius
              if (xCenter(i).le.sim_rvir) then
                lsum(8) = lsum(8) + solnData(DENS_VAR,i,j,k)*dvol/1.98E33
