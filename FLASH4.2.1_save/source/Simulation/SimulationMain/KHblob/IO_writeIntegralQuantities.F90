...
subroutine IO_writeIntegralQuantities ( isFirst, simTime)
  ...
  use Simulation_data, ONLY: sim_velambient, sim_tcc, sim_shiftTime, & 
      sim_rhoblob,sim_blobradius,sim_blobx,sim_bloby
...
#include "Flash.h"
  ...
  gsum(21) = sim_velambient
  gsum(22) = simTime/sim_tcc
     ...
     if (isfirst == 0) then
        ...
        if (.NOT. io_restart) then
           ...
           write (funit, 10)               &
                ...
                'mass blob (>rho0/3.)      ', &
                'mass blob (>rho0/10.)     ', &
                'x-velocity blob (>rho0/3.)', &
                'x-center blob   (>rho0/3.)', &
                'y-center blob   (>rho0/3.)', &
                'velambient                ', &
                'time/tcc                  '
