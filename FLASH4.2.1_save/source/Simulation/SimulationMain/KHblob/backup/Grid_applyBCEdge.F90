...
subroutine Grid_applyBCEdge(bcType,bcDir,guard,var,dataRow,face,&
  ...
  use Simulation_data
  use Hydro_data, ONLY : hy_smallp,hy_smallx
...
! print *, 'UD:', USER_DEFINED, ' ', 'HERE'
!      case default 
        select case(var)
        case(GAMC_VAR)
           dataRow(1:guard)=sim_gamma
        !   print *,'gamc',sim_gamma 
        case(GAME_VAR)
           dataRow(1:guard)=sim_gamma
        !   print *,'gvar',sim_gamma 
        case(DENS_VAR)
           dataRow(1:guard)=eosData(EOS_DENS)
        !   print *,'dens',sim_rhoRight
        case(PRES_VAR)
!           pRight =  sim_rhoambient*sim_tempambient*sim_grv_boltz/sim_amu/0.6
           dataRow(1:guard)= eosData(EOS_PRES)
        !   print *,'pRight',pRight
        case(K_VAR)
           dataRow(1:guard)= 1.0E-20
        case(L_MSCALAR)
           dataRow(1:guard)= 1.0E-20
        case(MUT_VAR)
	       dataRow(1:guard)= 1.0E-20*sqrt(2.*1.0E-20)
        case(KFRC_MSCALAR)
	       dataRow(1:guard)= 1.0E-20
        case(VELX_VAR)
           dataRow(1:guard)= sim_velambient
        !   print *,'Velx',-sim_uRight*sim_yCos
        case(VELY_VAR)
           dataRow(1:guard)=  0.! sim_velambient
        !   print *,'vely',sim_uRight*sim_xCos
        case(VELZ_VAR)
           dataRow(1:guard)=0.0
        case(ENER_VAR)
           kine = 0.5 * (sim_velambient**2)
           dataRow(1:guard) = pRight / ((sim_gamma-1.) * sim_rhoambient) + kine
        case(EINT_VAR)
           dataRow(1:guard) = eosData(EOS_EINT) !max(pRight / ((sim_gamma-1.) * sim_rhoambient), hy_smallp)
        case(TEMP_VAR)
	   dataRow(1:guard) = eosData(EOS_TEMP)
        end select
