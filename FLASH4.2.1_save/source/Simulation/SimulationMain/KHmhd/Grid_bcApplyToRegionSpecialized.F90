        ...
        if(face==LOW) then ! apply BC on the bottom boundary at x=0
           ...
           if (axis==IAXIS) then   ! IBEG 
              do i = 1,guard !x-direction
                 if (gridDataStruct==CENTER) then
                    regionData(i,1:je,1:ke,ivar) = regionData(guard+1,1:je,1:ke,ivar)
                    if (ivar == VELX_VAR) regionData(i,1:je,1:ke,ivar)= 0.0 
...
!ES HERE IT IS
               !if (gridDataStruct==CENTER) then
                  if (ivar == DENS_VAR) regionData(i,1:je,1:ke,ivar)= sim_rhoambient 
                  if (ivar == TEMP_VAR) regionData(i,1:je,1:ke,ivar)= sim_tempambient
                  if (ivar == PRES_VAR) regionData(i,1:je,1:ke,ivar)= sim_pres
                  ei = sim_pres/(sim_gamma-1)/sim_rhoambient
                  vy =  sim_velambient-sim_velframe 
                  if (ivar == EINT_VAR) regionData(i,1:je,1:ke,ivar)= ei
                  if (ivar == ENER_VAR) regionData(i,1:je,1:ke,ivar)= ei+0.5*vy*vy
                  if (ivar == VELX_VAR) regionData(i,1:je,1:ke,ivar)= 0.0 
                  if (ivar == VELY_VAR) regionData(i,1:je,1:ke,ivar)= vy 
                  if (ivar == VELZ_VAR) regionData(i,1:je,1:ke,ivar)= 0.0 
                  if (ivar == BLOB_MSCALAR) regionData(i,1:je,1:ke,ivar)= 0.0
                  if (ivar == MAGX_VAR) regionData(i,1:je,1:ke,ivar)= sim_bx
                  if (ivar == MAGY_VAR) regionData(i,1:je,1:ke,ivar)= sim_by
                  if (ivar == MAGZ_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
                  if (ivar == DIVB_VAR) regionData(i,1:je,1:ke,ivar)= 0.0
                  if (ivar == MAGP_VAR) regionData(i,1:je,1:ke,ivar)= sim_magp
                  if (gridDataStruct == FACEX) then
                    if (ivar == MAG_FACE_VAR) regionData(i,1:je,1:ke,ivar)  = sim_bx
                    if (ivar == MAGI_FACE_VAR) regionData(i,1:je,1:ke,ivar) = sim_bx
                  ! endif
                  elseif (gridDataStruct == FACEY) then
                    if (ivar == MAG_FACE_VAR) regionData(i,1:je,1:ke,ivar)  = sim_by
                    if (ivar == MAGI_FACE_VAR) regionData(i,1:je,1:ke,ivar) = sim_by
                  endif
              end do
           endif  
           case default
