!!****if* source/Simulation/SimulationMain/KHmhd/Grid_applyBCEdge
!!
!! NAME
!!  Grid_applyBCEdge
!!
...
!!  This implementation is specific to the KHmhd problem which 
...
# include "Eos.h"
  ...
  if (gridDataStruct/=CENTER) then
     print*,'boundary is',bcType,face,gridDataStruct
...
!      case default 
        ...
        case(VELX_VAR)
           dataRow(1:guard) = 0.
        case(VELY_VAR)
           dataRow(1:guard) = sim_velambient-sim_velframe
        case(VELZ_VAR)
           dataRow(1:guard) =0.0
        case(ENER_VAR)
           kine = 0.5 * (sim_velambient-sim_velframe)**2
           dataRow(1:guard) = pRight / ((sim_gamma-1.) * sim_rhoambient) + kine
        case(EINT_VAR)
           dataRow(1:guard) = eosData(EOS_EINT) 
        case(TEMP_VAR)
