...
#include "Flash_mpi.h"  
  ...
  integer :: periodic(3) !ES An array to implement periodic boundary contidions
  ...
  !!-----------------------------------------------------------------------
  !!     ES 10. Institute Periodic Boundary Conditions 
  !!-----------------------------------------------------------------------      
  periodic(1)=dr_axisNumProcs(1)*NXB
  periodic(2)=dr_axisNumProcs(2)*NYB
  periodic(3)=dr_axisNumProcs(3)*NZB
  print *,'CallingStructgrid',periodic(1),periodic(2),periodic(3)
  call HYPRE_SStructGridSetPeriodic(gr_hypreGrid,part,periodic,ierr)
  ...
  !!----------------------------------------------------------------------    
  nentries = NDIM*2 + 1   
  call HYPRE_SStructStencilCreate(NDIM, nentries, gr_hypreStencils, ierr)
  ...
  do ent = 1, nentries
     call HYPRE_SStructStencilSetEntry(gr_hypreStencils, ent-1, offsets(ent,1:NDIM), var, ierr)
  enddo
  ...
  call HYPRE_SStructGraphSetStencil(gr_hypreGraph, part, var, gr_hypreStencils, ierr)          
