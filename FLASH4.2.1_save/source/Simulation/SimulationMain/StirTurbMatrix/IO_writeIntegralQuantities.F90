...
subroutine IO_writeIntegralQuantities (isFirst, simTime)
  ...
  use Simulation_data, ONLY: tone,ttwo,tfour,teight,npdfstart, &
                             n_one,ntwo,nfour,neight,sim_rhoambient,sim_writematrix
...
!! then we write the pdf file again 
  if((dr_nstep .ge. 1*n_one).and.(dr_nstep .ge. npdfstart)) then
...
!   now do the case where you box smooth over spheres of various sizes
     do csmooth = 1,3
       istep = 1+csmooth
       istsq = istep*istep
                ...
                do jp=j-istep,j+istep
                  ...
                  do ip=i-istep,i+istep
                    ...
                    if(ksq+jsq+isq.le.istsq) then
                      ...
                      dsmoothold  = dsmoothold +solnData(ieul,ip,jp,kp)
                      dsmootholdm = dsmootholdm+solnData(DENS_VAR,ip,jp,kp)*solnData(imas,ip,jp,kp) 
                      totzones    = totzones + 1
                    endif
                  end do 
                end do 
              end do 
              dsmootholdm = dsmootholdm/dsmoothnew
              dsmoothold  = dsmoothold/totzones
              dsmoothnew  = dsmoothnew/totzones
              dsmoothnewijk(i,j,k) = dsmoothnew
...
!     write the old densities to the appropriate arrays 
        do k = blkLimits(LOW,KAXIS), blkLimits(HIGH,KAXIS)
          do j = blkLimits(LOW,JAXIS), blkLimits(HIGH,JAXIS)
             do i = blkLimits(LOW,IAXIS), blkLimits(HIGH,IAXIS)
              solnData(ieul,i,j,k)     = dsmoothnewijk(i,j,k)
              solnData(imas,i,j,k)     = dsmoothnewijk(i,j,k)
            end do
          end do
        end do
      end do 
    ...
    if (io_globalMe  == MASTER_PE) then
      ...
      if((sim_writematrix).or.(mod(n_one,16).eq.1)) then
        write (fnumStr, '(i4.4)') (n_one-npdfstart)
...
21  format (' ',I5.2)
  ...
  !! then we write the pdf file again 
  if((dr_nstep .ge. 2*ntwo).and.(dr_nstep .ge. npdfstart)) then
...
!     write the old one 
    ...
    if (io_globalMe  == MASTER_PE) then
      ...
      if((sim_writematrix).or.(mod(ntwo,8).eq.1)) then
        write (fnumStr, '(i4.4)') (ntwo-npdfstart/2)
...
!     write the s values
  ...
  !! then we write the pdf file again 
  if((dr_nstep .ge. 4*nfour).and.(dr_nstep .ge. npdfstart)) then
...
!     write the old one 
    ...
    if (io_globalMe  == MASTER_PE) then
      ...
      if((sim_writematrix).or.(mod(nfour,4).eq.1)) then
        ...
        write (fnumStr, '(i4.4)') (nfour-npdfstart/4)
...
!     write the s2 values
  ...
  !! then we write the pdf file again 
  if((dr_nstep .ge. 8*neight).and.(dr_nstep .ge. npdfstart)) then
...
!     write the old one 
    ...
    if (io_globalMe  == MASTER_PE) then
      ...
      if((sim_writematrix).or.(mod(neight,2).eq.1)) then
        write (fnumStr, '(i4.4)') (neight-npdfstart/8)
