...
#include "Eos.h"
  ...
  rvecSize = blkLimitsGC(HIGH,IAXIS)*blkLimitsGC(HIGH,JAXIS)*blkLimitsGC(HIGH,KAXIS)
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
	   ...
	   rhoZone = sim_rhoAmbient*& 
                     (1.0+sim_noise_amplitude*(1.0-2.0*rvec(icount)))
	   ...
	   solnData(DUST_MSCALAR,i,j,k) = 1.0
