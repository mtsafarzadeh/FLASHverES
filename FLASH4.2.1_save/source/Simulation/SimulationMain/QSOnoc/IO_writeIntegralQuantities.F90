...
#include "Flash_mpi.h"
  ...
  integer, parameter ::  nGlobalSum = 29   ! Number of globally-summed quantities
...
!! Volumne weighted jeans length
		...
		lsum(27) = lsum(27) + tcool* solnData(DENS_VAR,i,j,k)*dvol
                lfield   =sqrt( solnData(TEMP_VAR,i,j,k)*solnData(TEMP_VAR,i,j,k)/solnData(DENS_VAR,i,j,k)/&
                                abs(radi_gas+solnData(RADD_VAR,i,j,k)+solnData(RADR_VAR,i,j,k)))
...
11         format('# simulation restarted')
                        ...
                        rhorms = sqrt(gsum(21)/gsum(2)-0.*(gsum(1)*gsum(1)/  gsum(2)/gsum(2)))
                        TrmsV  = sqrt(gsum(22)/gsum(2)-0.*(gsum(13)*gsum(13)/gsum(2)/gsum(2)))
                        TrmsD  = sqrt(gsum(23)/gsum(1)-0.*(gsum(15)*gsum(15)/gsum(1)/gsum(1)))
