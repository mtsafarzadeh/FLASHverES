...
subroutine gr_markRefineDerefine(&
  ...
  use Simulation_data, ONLY: sim_blobradius,sim_tcc, sim_blobx, sim_bloby
...
!       This part adds a derefine in front and behind the blob
        blockCenter(:) = coord(:,lb)
        xdist = blockcenter(1)
        ydist = blockcenter(2)
        zdist = blockcenter(3)
        xdist = sqrt(xdist**2)
...
#if N_DIM == 3
        xdist = sqrt(xdist**2+zdist**2)
#endif

!        Derefine outside of 3 times the initial blob radius or 9 times the
!        current one 
        if((xdist> 3.*sim_blobradius).or.(xdist>9.*sim_blobx)) then 
          derefine(lb) = .TRUE.
          stay(lb) = .FALSE.
          refine(lb) = .FALSE.
        endif
!       If a cell outside of the initial blob radius or 3 times the current blob radius 
!       AND
!       If the cell is at least 3 times the initial blob radius away along the
!       flow access from both y=0 and the COM in the y direction.   Then
!       derefine
        if(((xdist> sim_blobradius).or.(xdist>3.*sim_blobx)).and. & 
            (((ydist<-3.*sim_blobradius).and.(ydist-sim_bloby<-3.*sim_blobradius)).or. &
             ((ydist> 3.*sim_blobradius).and.(ydist-sim_bloby> 3.*sim_blobradius)))) then
          derefine(lb) = .TRUE.
          stay(lb) = .FALSE.
          refine(lb) = .FALSE.
        endif
!       This part forces a refinment at the highest level around the blob itself       
!       for two cloud crushing times
        if((xdist<1.5*sim_blobradius).and. &
           ((abs(ydist)<1.5*sim_blobradius)).and.(dr_simtime.le.sim_tcc)) then
          derefine(lb) = .FALSE.
          stay(lb) = .FALSE.
          refine(lb) = .TRUE.
        endif
!       This part forces a refinment at the highest level around the blob itself       
        if((xdist<1.0*sim_blobradius).and. &
            (abs(ydist-sim_bloby)<1.0*sim_blobradius)) then 
          derefine(lb) = .FALSE.
          stay(lb) = .FALSE.
          refine(lb) = .TRUE.
        endif
