!!****if* source/Simulation/SimulationMain/magnetoHD/NohCylindrical_02/MYSETUP/Simulation_initBlock
!!
!! NAME
!!
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
        do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
           ...
           solnData(DENS_VAR,i,j,k)=  1.0 ! 12.18085*radius**5
           if (NDIM == 2) then
         ...
         ! plasma beta is 8 PI 10^-6
           ...
           solnData(MAGY_VAR,i,j,k)=  0.0!-xCoord(i)**2.5*4.999988*1.e5/sqrt(4.*PI*sim_UnitDensity*sim_UnitVelocity**2)
           solnData(MAGZ_VAR,i,j,k)=  0.0!xCoord(i)**2.5*5.0*1.e5/sqrt(4.*PI*sim_UnitDensity*sim_UnitVelocity**2)
           ...
           if (sim_killdivb) then
              if (NDIM == 2) then
                 ...
                 !-xCoord(i)**2.5*4.999988*1.e5/sqrt(4.*PI*sim_UnitDensity*sim_UnitVelocity**2)
                 solnData(MAGX_VAR,i,j,k)= 0.0
                 solnData(MAGY_VAR,i,j,k)= 0.0!-xCoord(i)**2.5*4.999988*1.e5/sqrt(4.*PI*sim_UnitDensity*sim_UnitVelocity**2)
                 solnData(MAGZ_VAR,i,j,k)= 0.0!xCoord(i)**2.5*5.0*1.e5/sqrt(4.*PI*sim_UnitDensity*sim_UnitVelocity**2)
              endif
