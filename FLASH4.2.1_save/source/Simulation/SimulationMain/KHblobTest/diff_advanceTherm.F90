...
#include "Eos_components.h"
  ...
  real    :: bcValues(2,6) = 0.
  real    :: cond_zone, diff_coeff, xdens, xtemp, xpres
     ...
     do k = blkLimits(LOW,KAXIS)-K3D, blklimits(HIGH,KAXIS)+K3D
        do j = blklimits(LOW,JAXIS)-K2D, blklimits(HIGH,JAXIS)+K2D
           do i = blklimits(LOW,IAXIS)-1, blklimits(HIGH,IAXIS)+1
              ...
              fm1 = solnVec(Temptodiffuse, i-1, j, k)
              fp1 = solnVec(Temptodiffuse, i+1, j, k)
              gradi = ((fp1-fm1)/(2.0*delta(IAXIS)))**2
              maggrad = gradi

#if NDIM >= 2
              fm1 = SolnVec(Temptodiffuse, i, j-1, k)
              fp1 = SolnVec(Temptodiffuse, i, j+1, k)
                 gradj = ((fp1-fm1)/(2.0*delta(JAXIS)))**2
              maggrad = maggrad + gradj
#endif

#if NDIM == 3
              fm1 = solnVec(Temptodiffuse, i, j, k-1)
              fp1 = solnVec(Temptodiffuse, i, j, k+1)
...
!                 call Conductivity(solnVec(:,i,j,k), cond_zone, component=EOSCOMP_ELE)
                 ...
                 solnVec(COND_VAR,i,j,k) = diff_eleFlCoef *sqrt(xpres/xdens) * xpres/(maggrad+1.e-14)
                 solnVec(DFCF_VAR,i,j,k) = xdens*eos_arr(cvToUse)

                 ! Set the flux limiter:
!                 call Eos_getAbarZbar(solnVec(:,i,j,k),Ye=Ye)
                 solnVec(FLLM_VAR,i,j,k) = maggrad
!diff_eleFlCoef * &
