...
#include "Flash.h"
  ...
  real :: enerZone, ekinZone, eintZone, sim_p
  real, pointer, dimension(:,:,:,:) :: solnData, facexData,faceyData,facezData

  sim_p = sim_rhoambient*sim_tempambient*1.38d-16/0.6/1.67d-24
...
!           if (xxR .lt. lposn) then
           if ((xxR .gt. 0.).and.(yy .gt. 0.)) then

              solnData(DENS_VAR,i,j,k)  = sim_rhoambient
              solnData(PRES_VAR,i,j,k)  = sim_p
!              solnData(TEMP_VAR,i,j,k)  = sim_tempambient
...
!              solnData(DENS_VAR,i,j,k)  = 0.5 * (sim_rhoambient+ sim_rhoblob)
!              solnData(PRES_VAR,i,j,k)  = sim_p
!              solnData(TEMP_VAR,i,j,k)  = 0.5*(sim_tempambient+(sim_rhoambient/sim_rhoblob)*sim_tempambient)
!              solnData(VELX_VAR,i,j,k)  = 0.
!              solnData(VELY_VAR,i,j,k)  = 0.
!              solnData(VELZ_VAR,i,j,k)  = 0.
...
!#endif
              ...
              solnData(DENS_VAR,i,j,k)  = sim_rhoblob
              solnData(PRES_VAR,i,j,k)  = sim_p
!              solnData(TEMP_VAR,i,j,k)  = (sim_rhoambient/sim_rhoblob)*sim_tempambient
              solnData(VELX_VAR,i,j,k)  = 0.
              solnData(VELY_VAR,i,j,k)  = 0.
              solnData(VELZ_VAR,i,j,k)  = 0.
