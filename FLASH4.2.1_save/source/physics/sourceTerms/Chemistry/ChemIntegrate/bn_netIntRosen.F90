!!****if* source/physics/sourceTerms/Burn/BurnIntegrate/bn_netIntRosen
!!
!! NAME
...
subroutine bn_rosenGift(y,dydx,n,x,htry,eps,yscal,hdid,hnext,  & 
  ...
  use chem_interface, ONLY: chem_gift
  use Chemistry_data
...
!     call lubksb(av,n-1,nmax,indx,bb)
   ...
   !!  compute the third and fourth order estimates of y 
     ...
     if (x .eq. xsav) then 
        ...
        if (myPE .EQ. MASTER_PE) print *, 'step size not significant in bn_rosenGift' 
        call Logfile_stampMessage(myPE,'[bn_rosenGift] step size not significant')
...
!!  too many tries
  ...
  if (myPE .EQ. MASTER_PE) then
      print *, 'ERROR exceeded maxtry in routine bn_rosenGift' 
  endif
  ...
  call Logfile_stampMessage(myPE,'[bn_rosenGift] ERROR exceeded maxtry')
