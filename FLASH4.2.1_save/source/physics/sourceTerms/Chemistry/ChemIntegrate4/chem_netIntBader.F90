...
subroutine Chemistry_netIntBader(y,dydx,nv,x,htry,eps,yscal,hdid,hnext, & 
     &                   derivs,jakob,jcounts,dens,temp)
...
!  end interface
  ...
  real, intent(IN)    :: dydx(nv), yscal(nv), htry, eps, dens, temp
  ...
  logical, save    :: reduct
  integer, parameter  ::    nmax = 70, kmaxx=7, imax=kmaxx+1   
  integer, parameter  ::    naij = 1000, n5 = 5*nmax, n8=8*nmax
...
end subroutine Chemistry_netIntBader
...
!!  NOTE jloc and ikeep are not used anywhere.....
  ...
  real, intent(IN)    :: y(n), dydx(n), dfdy(naij), xs, htot, dens, temp
  real, intent(INOUT) :: yout(n), w(1)
  real, intent(OUT)   :: a(naij)

!!  declare local variables
  integer, parameter ::     nmaxx=70 
  integer, save :: i,nn
  real, save ::    h,x,del(nmaxx),ytemp(nmaxx)


  !!print *,'IN CHEM_NETINTBADER'
