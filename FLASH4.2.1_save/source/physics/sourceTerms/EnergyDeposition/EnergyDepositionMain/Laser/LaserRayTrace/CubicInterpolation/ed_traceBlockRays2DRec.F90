!!****if* source/physics/sourceTerms/EnergyDeposition/EnergyDepositionMain/Laser/LaserRayTrace/CubicInterpolation/ed_traceBlockRays2DRec
!!
!! NAME
!!
...
!        gradients (d/dx and d/dy) at the current ray position in the (i,j) cell. These
...
!        ---------------------------------------------------------------------------
!
!        The bicubic expansion coefficients a (i,j) for each cell contain all the info that
!        is needed. For a point (X,Y) inside the cell with rescaled [0,1] x,y coordinates,
!        we have:
!
!
!                              3   3            i j
...
!
!                              3   3                i-1 j
!                     dF/dX = sum sum  i * a (i,j) x   y  / (cell x-dimension)
!                             i=1 j=0
!
!                              3   3                i j-1
!                     dF/dY = sum sum  j * a (i,j) x y    / (cell y-dimension)
!                             i=0 j=1
!
!
...
!
!        dF/dY  -->  The picture shows the entire vectors. Each box corresponds to
...
!
     ...
     xy ( 2)    = x                               !
     xy ( 3)    = x * xy (2)                      !               i j
     xy ( 4)    = x * xy (3)                      ! the monomial x y  array
     xy ( 5: 8) = y * xy (1:4)                    !
     xy ( 9:12) = y * xy (5:8)                    !
     xy (13:16) = y * xy (9:12)                   !
!
...
!
!
     Nele = sum (xy (1:16) * ed_cellCubicNele (1:16,i,j,1))

     a = sum ( xy (1:13:4) * ed_cellCubicNele (2:14:4,i,j,1))
     b = sum ( xy (2:14:4) * ed_cellCubicNele (3:15:4,i,j,1))
     c = sum ( xy (3:15:4) * ed_cellCubicNele (4:16:4,i,j,1))

     gradNeleX = (a + b + b + c + c + c) * deltaInvX

     a = sum (  xy ( 1: 4) * ed_cellCubicNele ( 5: 8,i,j,1))
     b = sum (  xy ( 5: 8) * ed_cellCubicNele ( 9:12,i,j,1))
     c = sum (  xy ( 9:12) * ed_cellCubicNele (13:16,i,j,1))
...
!
!
     Tele = sum (xy (1:16) * ed_cellCubicTele (1:16,i,j,1))

     a = sum ( xy (1:13:4) * ed_cellCubicTele (2:14:4,i,j,1))
     b = sum ( xy (2:14:4) * ed_cellCubicTele (3:15:4,i,j,1))
     c = sum ( xy (3:15:4) * ed_cellCubicTele (4:16:4,i,j,1))

     gradTeleX = (a + b + b + c + c + c) * deltaInvX

     a = sum (  xy ( 1: 4) * ed_cellCubicTele ( 5: 8,i,j,1))
     b = sum (  xy ( 5: 8) * ed_cellCubicTele ( 9:12,i,j,1))
     c = sum (  xy ( 9:12) * ed_cellCubicTele (13:16,i,j,1))
...
!        cell (i,j) or to one of the 4 cell walls. After this section, we have the correct
...
!
!
        stepVelocity = max (abs (velX), abs (velY))
        ...
        if (newCell) then
            ...
            if (stepTime == ed_infiniteTime) then
                call Driver_abortFlash ('[ed_traceBlockRays2DRec] ERROR: infinite crossing time for a cell')
            end if
