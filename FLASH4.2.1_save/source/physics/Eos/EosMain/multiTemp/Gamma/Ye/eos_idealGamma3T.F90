!!****if* source/physics/Eos/EosMain/multiTemp/Gamma/Ye/eos_idealGamma3T
!!
!!
!! NAME
!!
...
!!
!!  componentMask: Mask is a logical array the size of N_EOS_TEMP (number of components,
!!                 defined in Eos.h, typically either 1 -- if not using multiTemp code ---
!!                 or 3 for three-temperature approximation), where each index represents
!!                 a component (ions, electrons, and/or radiation) that contributes to
!!                 the EOS.
!!
!! PARAMETERS
!!
...
!==============================================================================
  ...
  use eos_idealGammaData, ONLY: eos_gamma, eos_gammam1, &
       ...
       eos_gammam1Ion, eos_gammam1Ele, eos_eMass, eos_eMassInUAmu
...
#include "Multispecies.h"
  ...
  select case (myType)
     case(EOS_GAM)
        ! ok...
     case default
        if (eos_meshMe==MASTER_PE) print*,"eos_idealGamma3T: only supports EOS type EOS_GAM, called for eosType=", myType
        call Driver_abortFlash("eos_idealGamma3T: only supports EOS_GAM, called for wrong EOS type!")
  end select
  ...
  if (present(componentMask)) then
     myCMask = componentMask
  else
     myCMask = overallCMask
     if (mode==MODE_DENS_TEMP_MAT_EQUI) then
        myCMask(3) = 0
     end if
  end if
  ...
  kBoltz = eos_kerg
  seleVariant = eos_entrEleScaleChoice        !Use 4,5,6,7; other values experimental/historical - KW

  densRef = eos_smallRho; densRefInv = 1.0/densRef
  tempRef = eos_smallT; tempRefInv = 1.0/tempRef
...
!============================================================================
  ...
  case(MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_GATHER)
     useNRForAddtlCombined = .FALSE.; useNRForGamcCombined = .FALSE.
  case(MODE_DENS_TEMP,MODE_DENS_TEMP_EQUI,MODE_DENS_TEMP_MAT_EQUI)
!!$     useNRForAddtlCombined = .TRUE.; useNRForGamcCombined = .TRUE. !NONO in this file!
  ...
  !!!! Electrons: <eos_gammaEle,eos_gammam1Ele,>[ggprodEle,ggprodinvEle,gamm1invEle]
  select case (mode)
  case(MODE_DENS_TEMP,MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_EQUI, &
       MODE_DENS_TEMP_MAT_EQUI,                                &
  ...
  !!!! Ions: <eosData(gamc+)/GAMMA/eos_gammaIon>->gamIon,gamM1Ion,[ggprodIon,ggprodinvIon,gamm1invIon]
  if (species > 0) then
     call Multispecies_getProperty(species,GAMMA,gamIonFromMsDb)
     gamM1Ion(ilo:ihi) = 1./(gamIonFromMsDb-1.0)
     ...
     case(MODE_DENS_TEMP,MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_EQUI, &
          MODE_DENS_TEMP_MAT_EQUI,                                &
          MODE_DENS_TEMP_ALL,MODE_DENS_TEMP_GATHER)
        !Take the gamc that was passed is as the gamma for ions only!
        ggprodIon(ilo:ihi) = gamM1Ion(ilo:ihi) * eos_gasConstant
     case(MODE_DENS_TEMP_ION,MODE_DENS_TEMP_ELE)
        ggprodIon(ilo:ihi) = gamM1Ion(ilo:ihi) * eos_gasConstant
     ...
     case(MODE_DENS_EI, MODE_DENS_EI_SCATTER, &
          ...
          MODE_DENS_EI_SELE_GATHER)
        ggprodIon(ilo:ihi) = gamM1Ion(ilo:ihi) * eos_gasConstant
        ggprodinvIon(ilo:ihi) = 1. / ggprodIon(ilo:ihi)
        gam1invIon(ilo:ihi)   = 1. / gamM1Ion(ilo:ihi)
        ! density, pressures taken as input:
     case (MODE_DENS_PRES)
     case default
        if (eos_meshMe==MASTER_PE) print*,"eos_idealGamma3T: unrecognized mode", mode
        call Driver_abortFlash("[eos_idealGamma3T] Unrecognized input mode given to Eos")
     end select
  else
     gamM1Ion(ilo:ihi) = eos_gammam1Ion
     gamIon(ilo:ihi) = eos_gammaIon
     select case (mode)
     case(MODE_DENS_TEMP,MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_EQUI, &
          MODE_DENS_TEMP_MAT_EQUI,                                &
          MODE_DENS_TEMP_ALL,MODE_DENS_TEMP_GATHER)
        ggprodIon(ilo:ihi) = eos_gammam1Ion * eos_gasConstant
        ! density, internal energies (or electron entropy) taken as input:
     case(MODE_DENS_EI, MODE_DENS_EI_SCATTER, &
          MODE_DENS_EI_COMP, MODE_DENS_EI_GATHER, &
          MODE_DENS_EI_RECAL_GATHER, &
        ...
        ggprodIon(ilo:ihi) = eos_gammam1Ion * eos_gasConstant
        ggprodinvIon(ilo:ihi) = 1. / ggprodIon(ilo:ihi)
        gam1invIon(ilo:ihi)   = 1. / eos_gammam1Ion
        ! density, pressures taken as input:
     case (MODE_DENS_PRES)
     case default
        if (eos_meshMe==MASTER_PE) print*,"eos_idealGamma3T: unrecognized mode", mode
  ...
  !WAS Combined (Electrons+Ions):<f(gamIon,eos_gammaEle,Z)/eos_gamma>->eosData(gamc+),<???!!/eos_gammam1,>[ggprod,ggprodinv]
  !!!! Combined (Electrons+Ions):<f(gamIon,eos_gammaEle,Z)>->eosData(gamc+),<f(gamIon,eos_gammaEle,Z)/eos_gamma>->1./(gamEComb-1),[ggprod,ggprodinv]
  ...
  if (species > 0) then
     ggprod(ilo:ihi) = eos_gasConstant / (eosData(gamc+ilo:gamc+ihi) - 1.0)
     select case (mode)
     case(MODE_DENS_TEMP,MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_EQUI, &
          MODE_DENS_TEMP_MAT_EQUI,                                &
          MODE_DENS_TEMP_ALL,MODE_DENS_TEMP_GATHER)
     ...
     case(MODE_DENS_EI, MODE_DENS_EI_SCATTER, &
          ...
          MODE_DENS_EI_ALL, &
          MODE_DENS_EI_SELE_GATHER)
        ggprodinv(ilo:ihi) = 1. / ggprod(ilo:ihi)
     case (MODE_DENS_PRES)
     case default
        call Driver_abortFlash("[eos_idealGamma] Unrecognized input mode given to Eos")
     end select
  else
     ggprod(ilo:ihi) = eos_gammam1 * eos_gasConstant
!!$     do i = ilo,ihi
!!$        eosData(gamc+i) = eos_gamma ! presume not called from eos_multiTypeByTemp
!!$        ! DEV: If NOT using Ye, use eos_singleSpecies? RPs in place of next 2 lines - KW
!!$        !     eosData(abar+i)  --  use values passed in by caller
!!$        !     eosData(zbar+i)  --  use values passed in by caller
!!$     end do
     select case (mode)
     case(MODE_DENS_TEMP,MODE_DENS_TEMP_COMP,MODE_DENS_TEMP_EQUI, &
          MODE_DENS_TEMP_ALL,MODE_DENS_TEMP_GATHER)
...
!!$          eosData(eint+ilo:eint+ihi) * gam1inv                        !overridden below!
     eosData(temp+ilo:temp+ihi) = eosData(eint+ilo:eint+ihi) * ggprodinv(ilo:ihi) * &
  ...
  case (MODE_DENS_EI_COMP, MODE_DENS_EI_GATHER, MODE_DENS_EI_RECAL_GATHER, MODE_DENS_EI_ALL)
     ...
     eosData(presIon+ilo:presIon+ihi) = (1 * eosData(dens+ilo:dens+ihi)) * &
          eosData(eintIon+ilo:eintIon+ihi) * gam1invIon(ilo:ihi)
     eosData(presEle+ilo:presEle+ihi) = (1 * eosData(dens+ilo:dens+ihi)) * &
          eosData(eintEle+ilo:eintEle+ihi) * gam1invEle

     eosData(tempIon+ilo:tempIon+ihi) = eosData(eintIon+ilo:eintIon+ihi) * ggprodinvIon(ilo:ihi) * &
          eosData(abar+ilo:abar+ihi)
...
!     gam1inv   = 1. / eos_gammam1 !currently not used
     eosData(eint+ilo:eint+ihi) = eosData(pres+ilo:pres+ihi) / &
                                   ( (gamEComb(ilo:ihi)-1) * eosData(dens+ilo:dens+ihi) )
     eosData(temp+ilo:temp+ihi) = eosData(eint+ilo:eint+ihi) * ggprodinv(ilo:ihi) * &
                                   eosData(abar+ilo:abar+ihi) * ZpInv
     eosData(entr+ilo:entr+ihi) = 0.0
...
!     eosData(gamc+ilo:gamc+ihi)=gamcRow(1:rowLen) !replace with expression based on component states!
  ...
  if(present(mask)) then
     ...
     if(mask(EOS_DPD)) then
        ...
        eosData(dpd+ilo:dpd+ihi) = eos_gasConstant*eosData(temp+ilo:temp+ihi) &
             * Zp / eosData(abar+ilo:abar+ihi)
     end if
     if(mask(EOS_DET))then
        ...
        if(useNRForAddtlCombined) then
           ...
           call averageVectRByPressureFraction(det, &
                eosData(eintIon+ilo:eintIon+ihi) / eosData(tempIon+ilo:tempIon+ihi), &
                eosData(eintEle+ilo:eintEle+ihi) / eosData(tempEle+ilo:tempEle+ihi), &
                4 * myCMask(3)*eosData(eintRad+ilo:eintRad+ihi) / eosData(tempRad+ilo:tempRad+ihi))
       ...
       if(mode==MODE_DENS_TEMP_ELE) then
           ...
           case(1)              !ok?
              eosData(dst+ilo:dst+ihi) = &
                   kBoltz * &
                   eos_gammam1Ele / eosData(tempEle+ilo:tempEle+ihi)
           case(2)              !ok
              eosData(dst+ilo:dst+ihi) = &
...
!!$              eosData(dst+ilo:dst+ihi) = &
!!$                   Ye_ARRAYREFERENCE * &
!!$                   ( ( eos_gammam1Ele*(eosData(tempEle+ilo:tempEle+ihi)*tempRefInv*eos_eMassInUAmu)**(eos_gammam1Ele-1) * &
!!$                       tempRefInv*eos_eMassInUAmu ) &
!!$                   / ( eosData(dens+ilo:dens+ihi)*densRefInv  * Ye_ARRAYREFERENCE) ) / &
!!$                   ( (eosData(tempEle+ilo:tempEle+ihi)*tempRefInv*eos_eMassInUAmu)**eos_gammam1Ele &
!!$                     / ( eosData(dens+ilo:dens+ihi)*densRefInv  * Ye_ARRAYREFERENCE) )
              eosData(dst+ilo:dst+ihi) = &
                   Ye_ARRAYREFERENCE * &
                   eos_gammam1Ele / eosData(tempEle+ilo:tempEle+ihi)
           case(7)              !ok
              eosData(dst+ilo:dst+ihi) = eos_gammam1Ele / eosData(tempEle+ilo:tempEle+ihi)
           case(8)              !ok
              eosData(dst+ilo:dst+ihi) = &
                   eos_gammam1Ele*eosData(tempEle+ilo:tempEle+ihi)**(eos_gammam1Ele-1.0) &
                   /    eosData(dens+ilo:dens+ihi)
           case default
              eosData(dst+ilo:dst+ihi) = &
                   Ye_ARRAYREFERENCE * &
...
!!$           print*,'dst:',eosData(dst+ilo:dst+ihi)
       else
...
!!$           print*,'DST:',eosData(dst+ilo:dst+ihi)
!!$           print*,'DENS',eosData(dens+ilo:dens+ihi)
!!$           print*,'TEMP',eosData(temp+ilo:temp+ihi)
!!$           print*,'Pele',eosData(presEle+ilo:presEle+ihi)
!!$           print*,'Pion',eosData(presIon+ilo:presIon+ihi)
!!$           print*,'PRES',eosData(pres+ilo:pres+ihi)
        else
...
contains
  subroutine setEleEntropy(entrComponent,tempComponent)
    ...
    case(1)
       prefactor = kBoltz
    case(2,3)
       prefactor = eos_kergavo * Ye_ARRAYREFERENCE
    case(4,6)
       prefactor = Ye_ARRAYREFERENCE
    case(5,7)
       prefactor = 1.0
    case default
    ...
    if (coupled) then
       eosData(gamcCombined+ilo:gamcCombined+ihi) = &
         ( gamM1Ion(ilo:ihi)*eosData(presIon+ilo:presIon+ihi)+ &
           ...
           myCMask(3)*eos_gammam1Rad*eosData(presRad+ilo:presRad+ihi) ) &
           / eosData(pres+ilo:pres+ihi)
       eosData(gamcCombined+ilo:gamcCombined+ihi) = &
       ...
       eosData(gamcCombined+ilo:gamcCombined+ihi) = &
         ( gamIon(ilo:ihi)*eosData(presIon+ilo:presIon+ihi)+ &
           ...
           myCMask(3)*eos_gammaRad*eosData(presRad+ilo:presRad+ihi) ) &
           / eosData(pres+ilo:pres+ihi)
    end if
  ...
  !  that it's not worth the small storage save.
    ...
    rowEnd = rowLen
#ifdef DEBUG_DEC2010
    print*,'NR-I',eosData(eintIon+ilo:eintIon+ihi)
!    print*,'NR-e',eosData(eintEle+ilo:eintEle+ihi)
!    print*,'NR-R',eosData(eintRad+ilo:eintRad+ihi)
#endif
...
#ifdef DEBUG_DEC2010
    print*,'1.CT',tempRow(1:rowLen)
    print*,'1.iT',tempIonRow(1:rowLen)
    print*,'1.eT',tempEleRow(1:rowLen)
    print*,'1.rT',tempRadRow(1:rowLen)
#endif
    call eos_byTempMG(rowBegin,rowEnd,gamM1Ion(ilo:ihi),eosMask,componentMask=cMask,ggprodEle=ggprodEle)
...
#ifdef DEBUG_DEC2010
          print*,'k,tnew(k),(etotRow(k) - ewantRow(k)),detRow(k)', k,tnew(k),(etotRow(k) - ewantRow(k)), detRow(k)
#endif
