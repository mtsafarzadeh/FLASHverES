!!****if* source/physics/Eos/EosMain/multiTemp/Multigamma/eos_byTempMG
!!
!! NAME
!!
!!  eos_byTempMG
...
!! SYNOPSIS
!!
!!  call eos_byTempMG(integer(IN) :: eos_jlo,
!!                    integer(IN) :: eos_jhi,
!!                       real(IN) :: gamM1Ion(eos_jlo:eos_jhi),
...
!!  gamM1Ion -- Array with values 1/(gammaIon(j)-1), where gammaIon(j) is
!!              the diabatic index applicable to the ion gas of cell j.
...
!! NOTES
!!
!!  This interface differs from eos_byTemp by the additional gamM1Ion argument.
!!  In eos_byTemp, the ions are treated as an ideal gas with only translational
!!  degrees of freedom.
!!  In eos_byTempMG, the ions are treated as an ideal gas, but the number of
!!  degrees of freedom can vary from cell to cell and is given by gamM1Ion.
!!
...
!!  no matter whether they call eos_byTempMG with or without the optional arguments.
!!***
...
!  call Timers_start("eos_byTempMG")
  ...
  do j=eos_jlo,eos_jhi
     ...
     eion    = fHalf * pion * deni
     deiondd = (fHalf * dpiondd - eion)*deni
     deiondt = fHalf * xni * eos_kerg *deni

     if (bAprox13t) then
        dxnida  = -xni*ytot1 !Calhoun
        dpionda = dxnida * ktIon !Calhoun
        deionda = fHalf * dpionda*deni  !Calhoun / KW
     ...
     dsiondt = (dpiondt*deni + deiondt)*tempi   &
          ...
          + fHalf * kavoy * tempi !check formula - kw??
...
!   call Timers_stop("eos_byTempMG")
#ifdef DEBUG_DEC2010
  print*,'eos_byTempMG ret:detRow:',detRow(eos_jlo:eos_jhi)
#endif
