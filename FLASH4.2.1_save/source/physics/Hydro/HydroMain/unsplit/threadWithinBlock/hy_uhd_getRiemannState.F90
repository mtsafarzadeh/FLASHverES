...
#include "UHD.h"
  ...
  !$omp default(none) &
  !$omp shared(i0,j0,k0,imax,jmax,kHydro,kmax,k2,k3,scrch_Ctr,normalFieldUpdate,&
  !$omp blockID,blkLimitsGC,dt,del,ogravX,ogravY,ogravZ,FlatCoeff,hy_SpcR,hy_SpcL,hy_SpcSig,&
  !$omp hy_useGravity,U,Bx,By,Bz,hy_useGravHalfUpdate,transOrder3d,leftEig,rghtEig,lambda,&
  !$omp hy_order,hy_flattening,FlatTilde,blkLimits,hdt,hy_shockDetectOn,&
  !$omp hy_use3dFullCTU,hy_geometry,hy_useHybridOrder,&
  !$omp xCenter,DivU,kGrav,sig,hy_forceHydroLimit,hy_killdivb,datasize) &
  !$omp private(i,j,k,iDim, un,cf,Vc,order,Wn,Wp,sigmptr,sigcptr,sigpptr,&
