!!****if* source/physics/Hydro/HydroMain/unsplit_old/Hydro_Unsplit/threadBlockList/hy_uhd_unsplit
!!
!! NAME
!!
!!  hy_uhd_unsplit
...
#include "UHD.h"
...
#ifndef FLASH_UHD_NEED_SCRATCHVARS
  call Driver_abortFlash("The tbl multithreading is incompatible with this mode")
  !CD: A solution is to mark the hy_scrchCtr array threadprivate in Hydro_data
  !when ifndef FLASH_UHD_NEED_SCRATCHVARS.  I don't like this solution because
  !I would need two ifdefed OpenMP parallel regions in this file since
  !scrch_Ctr is scoped differently.  There is also the issue that absoft, PGI &
  !XLF (on BG/Q) compilers have trouble with threadprivate data.
  !Lets see if this mode sticks around in FLASH first.
#endif
  ...
  !$omp flx,fly,flz,gravX,gravY,gravZ,U,scrch_Ctr,faceAreas) &
  ...
  !$omp hy_useGravPotUpdate,hy_geometry,hy_fluxCorVars,hy_needScrchVars,&
  !$omp halfTimeGravUpdate, hy_3TMode)
...
!!                                                                                                      !
  ...
  enddo
  !$omp end do                                                                                          !
  !CD: MATCHES LINE 224 (FIRST LOOP OVER BLOCKS)                                                        !
                                                                                                        !
                                                                                                        !
                                                                                                        !
#ifdef GPOT_VAR
...
!!\ RETURNING TO CONTINUE THE FIRST BLOCK DO-LOOP FOR GRAVITATIONAL POTENTIAL UPDATE ===================/
  ...
  !$omp end do
  !CD: MATCHES LINE 224 (FIRST LOOP OVER BLOCKS)
  !CD: MATCHES LINE 389 (SECOND LOOP OVER BLOCKS) FOR FLASH_UHD_NEED_SCRATCHVARS
  ...
  if (hy_fluxCorrect) then
     ...
     !CD - Grid_conserveFluxes is not thread safe so call with a single
     !thread (implicit barrier after the single section).  We also depend
     !on the implicit barrier at the end of the previous parallel do...  
     !don't be tempted to put a nowait clause there!!
     !$omp single
     ...
     !$omp end do nowait
     !CD: MATCHES LINE 570 (THIRD LOOP OVER BLOCKS)
