!!****if* source/flashUtilities/interpolation/cubic/ut_triCubicFd1
!!
!! NAME
!!
!!  ut_triCubicFd1
...
!! SYNOPSIS
!!
!!  ut_triCubicFd1 (real, intent (in) :: a (1:64),
!!                  real, intent (in) :: x,
!!                  real, intent (in) :: y,
!!                  real, intent (in) :: z)
!!
!! DESCRIPTION
!!
!!  Calculates the function and rescaled derivative value for a triple [x,y,z] of
!!  rescaled [0,1] coordinates and the 64 tricubic expansion coefficients. The tricubic
...
!!
!!           ut_triCubicFd1 (1) = the function value
!!           ut_triCubicFd1 (2) = the rescaled d/dx value
!!           ut_triCubicFd1 (3) = the rescaled d/dy value
!!           ut_triCubicFd1 (4) = the rescaled d/dz value
!!
!!  2) The code checks, if the supplied triple [x,y,z] is rescaled.
!!
!!***

function ut_triCubicFd1 (a,x,y,z)
  ...
  real    :: ut_triCubicFd1 (1:4)     ! declares the function as an array
...
!
  ...
  if (notRescaled) then
      call Logfile_stamp     (x, ' = rescaled x coordinate [ut_triCubicFd1]'        )
      call Logfile_stamp     (y, ' = rescaled y coordinate [ut_triCubicFd1]'        )
      call Logfile_stamp     (z, ' = rescaled z coordinate [ut_triCubicFd1]'        )
      call Driver_abortFlash ('[ut_triCubicFd1] ERROR: [x,y,z] triple not rescaled!')
  end if
!
!
...
!
!
  ut_triCubicFd1 (1) = sum (xyz (1:64) * a (1:64))
!
!
...
!
  ...
  ut_triCubicFd1 (2) = p + q + q + r + r + r    ! this is d/dx
  ...
  ut_triCubicFd1 (3) = p + q + q + r + r + r    ! this is d/dy

  p = sum (  xyz ( 1:16) * a (17:32) )
  q = sum (  xyz (17:32) * a (33:48) )
  r = sum (  xyz (33:48) * a (49:64) )

  ut_triCubicFd1 (4) = p + q + q + r + r + r    ! this is d/dz
!
!
!     ...Ready!
!
