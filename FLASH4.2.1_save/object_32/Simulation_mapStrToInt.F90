...
#include "Flash.h"
    ...
    case("var1")
        select case(map)
        case(((MAPBLOCK_SCRATCH_CENTER * MAPBLOCKSIZE)+  4)/MAPBLOCKSIZE); key = (MAPBLOCK_SCRATCH_CENTER * MAPBLOCKSIZE)+  4
        end select
    case("deld")
        select case(map)
    ...
    case("var2")
        select case(map)
        case(((MAPBLOCK_SCRATCH_CENTER * MAPBLOCKSIZE)+  5)/MAPBLOCKSIZE); key = (MAPBLOCK_SCRATCH_CENTER * MAPBLOCKSIZE)+  5
        end select
    case("f02xmom")
        select case(map)
